...
volatile uint8_t Buffer[16]
    @0x700; // 0x700...0x70F Display buffer, upper row first, bit 7 = left column(user writes here)
volatile uint8_t AuxBuffer[16]
    @0x710;                        // 0x710...0x71F Aux buffer(not displayed by interrupt display refresh, used by user)
